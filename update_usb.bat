...
if %FOUND_COUNT%==0 (
    ...
    echo （條件：根目錄同時有 software\、software_catalog.json、AutoQuickSetup.exe^)
    echo.
    set /p "TARGET=請手動輸入隨身碟路徑（例 D:\）或直接關閉視窗："
    if "!TARGET!"=="" exit /b
...
for %%F in (.gitignore main.py README.md sync.bat sync_to_drive.bat test_main.py version.json) do (
    if exist "%TARGET%%%F" (
        del /f /q "%TARGET%%%F"
        echo   已刪除 %%F
    )
